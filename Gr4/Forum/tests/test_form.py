...
class test_employee_form(TestCase):
    ...
    def test_department_form_invalid(self):
        ...
        department_blank_form = form.save()
        self.assertNotEqual(department_blank_form.name , 'Test')


    def test_department_form_blank(self):

        department_blank_form = DepartmentForm(data={})
        self.assertFalse(department_blank_form.is_valid())
        self.assertNotEquals(len(department_blank_form.data), 1)
...
class test_comment_form(TestCase):
    ...
    def test_comment_form_valid(self):
        ...
        form = CommentForm(data={'text': "default comment"})
        self.assertTrue(form.is_valid())
        self.assertEqual(form.is_valid(), True)
        comment_valid_form = form.save()
        self.assertEqual(comment_valid_form.text,"default comment")
    ...
    def test_comment_form_invalid(self):
        ...
        self.assertEqual(form.is_valid(), True)
        comment_valid_form = form.save()
        self.assertNotEqual(comment_valid_form.text,"other comment")
    
    def test_comment_form_blank(self):

        comment_blank_form = CommentForm(data={})
        self.assertFalse(comment_blank_form.is_valid())
        self.assertNotEqual(len(comment_blank_form.data), 1)
